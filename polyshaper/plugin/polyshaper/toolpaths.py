...
"""
Polyshaper tool path generation
"""
...
class CuttingToolPathsGenerator(object):
    ...
    identical to the initial one where the first point is (0, 0) followed by the point closest to
    (0, 0). If there is a border, also the path to cut the border is added and the first point from
    the path is taken closest to the border bottom left point. The last point of the path is always
    (0, 0).
    """

    def __init__(self, input_path, close_distance, border=None):
        """ Constructor
        ...
        :param border: the border to cut or None
    ...
    def generate(self):
        ...
        start_point = point_path_squared_distance(bottom_left, self.input_path)[1]
        self.tool_path += rotate_closed_path(self.input_path, start_point)

        self.add_final_points()

    def get_bottom_left(self):
        """ Returns the bottom left point

        :return: the bottom left point
        :rtype: 2D point (couple of floats)
        """

        return self.border.bottom_left() if self.border else (0, 0)

    def add_initial_points(self):
        """ Adds initial points to the path
        """

        self.tool_path = [(0, 0)]

        if self.border:
            self.add_bottom_left_border_if_not_origin()

    def add_final_points(self):
        """ Adds the final points to the path

        This includes the border if present
        """

        if self.border:
            # Add instructions to cut border
            self.tool_path.append(self.border.bottom_left())
            self.tool_path.append(self.border.bottom_right())
            self.tool_path.append(self.border.top_right())
            self.tool_path.append(self.border.top_left())
            self.add_bottom_left_border_if_not_origin()

        self.tool_path.append((0, 0))

    def add_bottom_left_border_if_not_origin(self): # pylint: disable=invalid-name
        """ Adds to tool path the bottom left border point if different from (0, 0)
        """

        if self.border.bottom_left() != (0, 0):
            self.tool_path.append(self.border.bottom_left())
